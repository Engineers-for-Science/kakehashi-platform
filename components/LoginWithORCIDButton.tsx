...
const LoginWithORCIDButton = () => {
  const router = useRouter()

  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<boolean>(false);

  const loginWithORCIDButtonClickHandler = async () => {
    setLoading(true);
    ...
    if (data.needUpdateProfile) {
      router.push(`/create-profile?id=${data.userId}&name=${data.nameFromProvider}&email=${data.emailFromProvider}`)
    } else {
      router.push('/home')
    }
  }
  ...
  return (
    <Button
      onClick={loginWithORCIDButtonClickHandler}
      disabled={loading}
